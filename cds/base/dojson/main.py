    ...
    if test:
        for xml_input, json_output in zip(blob, parsed_record):
            ...
            for key, value in reverted_translation.iteritems():
                matched_key = None
                for input_key in xml_input.keys():
                    if re.match(key, input_key):
                ...
                similar_keys.add(matched_key)
                if reverted_translation[key] != xml_input[matched_key]:
                    different_keys_number += 1
                    different_keys.append((reverted_translation[key], xml_input[matched_key]))
...
def translate_json_to_marc(json_file, album=False, test=False):
    ...
    if album:
        parsed_record = [album_tomarc21.do(data) for data in loaded_json]
    else:
        parsed_record = [tomarc21.do(data) for data in loaded_json]
    return parsed_record
